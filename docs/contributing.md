...
- **MHCCL** — ported as a 1-D ResNet-18 pair sharing `Conv1dResNetEncoder`, for the reasons given under SimCLR. FINCH clustering is reimplemented in torch on the input's device: the reference uses scikit-learn, SciPy and pynndescent, none of which this library depends on, and a host round-trip per level would dominate a per-batch clustering step; component labelling is asserted equal to `scipy.sparse.csgraph` in `tests/unit/test_mhccl.py`. The reference clusters the whole training set once per epoch and addresses the assignments through a dataset-global sample index, which this library's batch contract does not carry, so clustering runs per batch over the raw and augmented embeddings unioned with a momentum feature bank; the bank is capped at one epoch of enqueued rows, since copies of the same sample from different encoder states cluster together and add a partition encoding duplication rather than structure. Further departures, each commented at its line: scoring uses `einsum("bd,bmd->bm", ...)` where the reference reshapes its candidate stack to the transposed shape, which reinterprets the buffer rather than permuting axes and leaves its einsum contracting mismatched elements; negatives are every instance outside the anchor's cluster, where the reference excludes only the positives it sampled and so leaves a large cluster's unsampled members available as negatives of an anchor they share a cluster with; unfillable candidate slots are masked out of the mean rather than raising, which the reference does whenever a batch holds fewer candidates than requested; centroids are computed without mutating the caller's feature matrix and divide by true per-cluster counts, where the reference zeroes rows in place and subtracts a constant one from every cluster; the level-to-level cluster map is returned by the merge step rather than recovered by scanning the label matrix per anchor per candidate; shuffle-BN and the `torch.distributed` dependency are dropped, being the identity at world size 1 and unnecessary under the `GroupNorm` default; and where the reference's CLI defaults and its published command disagree, the published command wins, which sets `use_projection_mlp` and `use_lr_scheduler`.
